...
This document describes steps to run a compartment model (Mukherjee & Guertin, 2025) on nascent transcript datasets to estimate changes in pause release and initiation. The model equations (Mukherjee & Guertin, 2025) provide direct estimates of changes in pause release and initiation. 
...
For direct install:
```{r, eval=FALSE}
pip install compartmentModel
```

We use virtual environment `venv` (recommended), but direct installation is also possible without setting up a `venv`:-
```{r, eval=FALSE}
cd /path/to/your/directory
python3 -m venv compModelEnv # name of the environment
source compModelEnv/bin/activate # activate the environment
python3 -m pip install compartmentModel # can directly install, without running `venv` steps above
```
...
The test files in this section belong to Triptolide dataset (12.5 mins).

* control data, `testData_TRP12min_control.txt`: raw file [link](https://raw.githubusercontent.com/guertinlab/compartment_model/refs/heads/main/compModel_vignette/testData_TRP12min_control.txt).
* treatment data, `testData_TRP12min_control.txt` : raw file [link](https://raw.githubusercontent.com/guertinlab/compartment_model/refs/heads/main/compModel_vignette/testData_TRP12min_treatment.txt).
* control all genes data for, `testData_TRP12min_All_control_genes.txt`: raw file [link](https://raw.githubusercontent.com/guertinlab/compartment_model/refs/heads/main/compModel_vignette/testData_TRP12min_All_control_genes.txt).

Please refer to section `Preparation of Inputs` for creation of these input files. All input files are also present in the [Github directory](https://github.com/guertinlab/compartment_model/tree/main/compModel_vignette). 

We need two tab-separated files from each of control and treatment conditions containing 'gene', 'pause_sum', 'body_density' for all genes of our interest. We also need a file containing pause sums of all expressed genes in baseline, in order to estimate an occupancy value (see Mukherjee & Guertin, 2025). Please refer to the section `Preparation of Inputs` for detailed overview of creation of these files. For this example, we will use two files prepared from Triptolide treatment data (Jonkers et al. 2014):-

```{r, engine='bash',  comment=''}
head testData_TRP12min_control.txt
```
```{r, engine='bash',  comment=''}
head testData_TRP12min_treatment.txt
```
## Running the model
...
```{r, eval=FALSE}
polcomp estparam --basename baseline --basefile testData_TRP12min_control.txt --exprname TRP_12.5min --exprfile testData_TRP12min_treatment.txt --allgenesfile testData_TRP12min_All_control_genes.txt --conf TRP_UpperLimitFile --outputdir paramFit_TRP_EntireRange_percentile
```
...
The output file `paramEstimationResults_TRP_12.5min_vs_baseline.txt` contains normalized pause and body densities for all the genes, along with different rates that can be calculated using these normalized values (see Mukherjee & Guertin, 2025 for equations). The number suffixes to `kelong` columns denote elongation speed in bp/seconds :-
```{r, engine='bash',  comment=''}
head -n4 paramFit_TRP_EntireRange_percentile/paramEstimationResults_TRP_12.5min_vs_baseline.txt
```
A way to display the parameter results is mentioned in the section `Plotting Rates`. The user may use custom functions for displaying data in `paramEstimationResults_TRP_12.5min_vs_baseline.txt`. `_normalized_` in column names refers to transformed pause and body densities to occupancy values (as mentioned in Mukherjee & Guertin, 2025).  Please refer to section, `Saturation values` where running `polcomp` with `--norm saturation` is discussed.
...
```
We now have desired files - `testData_TRP12min_control.txt` and `testData_TRP12min_treatment.txt` containing pause sums and body densities of genes of interest. We will use `testData_TRP12min_All_control_genes.txt` to estimate a full occupancy value for 1 RNAPII in the pause region.

## Saturation values

Note: As discussed in original manuscript (Fig S1, Mukherjee et al), we used pause sum at 90th percentile of data `testData_TRP12min_All_control_genes.txt` as occupancy value. See section `Running the model`.
...
```{r, eval=FALSE}
polcomp estparam --basename baseline --basefile testData_TRP12min_control.txt --exprname TRP_12.5min --exprfile testData_TRP12min_treatment.txt --allgenesfile testData_TRP12min_All_control_genes.txt --norm saturation --conf TRP_UpperLimitFile --outputdir paramFit_TRP_EntireRange_Saturation
```
...
The `generalPlotter.R` file - [link](https://raw.githubusercontent.com/guertinlab/compartment_model/refs/heads/main/compModel_vignette/generalPlotter.R)  is present in the vignette folder and is omitted here for brevity.
...
originalPauseSums.TRP =  read.table("testData_TRP12min_All_control_genes.txt", sep="\t", header=T)
...
# Generating all figures in the manuscript 

A long file `generateAllFigures.R` - [Link](https://raw.githubusercontent.com/guertinlab/compartment_model/refs/heads/main/compModel_vignette/generateAllFigures.R) has the source code to generate figures in manuscript Mukherjee & Guertin, 2025. It is located in the vignette folder, and the contents are omitted for brevity.
