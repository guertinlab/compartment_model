...
#' @param input a data frame with three columns in this order - index, y.val and x.val. y.val is plotted against x.val within the function    
#' @param filename this name is used to save the plot
#' @return NULL
...
#' @param input a data farme containing Body and Pause columns with corresponding values
#' @param tau a parameter which acts as a "time constant" for the exponential function fitting in find.body.param
#' @param min.pk minimum peak value - to be passed to [find.body.param()]
#' @param max.px maximum peak value - to be passed to [find.body.param()]
#' @param dpk threshold for implicit solution - to be passed to [find.body.param()]
#' @param gen.len length of gene i.e; x-range for the desired PRO-seq profile
...
#' @param input contains iteration, body and pause columns for values for plotting
#' @param filename name of the file to which the plot is saved
#' @return NULL
...
#' @param input a dataframe where first column is condition (group), second column is y.val, third column is x.val. 
#' @param pause.offset an offset to place vertical guide lines at [0 - pause.offset, 50 - pause.offset] 
#' @param filename name of the file to which the plot will be saved 
#' @param ylim two values to set limits of the y.axis
#' @param trace.col two set of colors used for plotting PRO-seq signal. Color indices will be mapped in alphabetical order of condition names.   
#' @return NULL
...
#' @param input a dataframe containing iteration, pause and body signal
#' @param tau a "time constant" parameter passed to [find.body.param()]
#' @param min.pk minimum of the peak value - to be passed to [find.body.param()]
#' @param max.pk maximum of the peak value - to be passed to [find.body.param()]
#' @param dpk threshold for implicit solution - to be passed to [find.body.param()]
#' @param gene.len range of the x-axis to be used for plotting.
#' @param pause.height height of the pause summit.  
#' @param time.char character which describes the condition
#' @return a dataframe with columns iteration, PRO-seq signal, x-coordinate 
...
#' a helper function to plot PRO-seq waveform derived after simulations. Used in the script plot_simulation_composites.R. Uses data from changing knit and krel and arranges code present from page 40 of compartment modeling vignette.
#' @param input.data.identifier a string which is used to identify data objects that are read. Ideally, the data objects were generated using [parallel.change.two.param.helper()] called in paramFittingCompartmentModel.R
#' @param output.plot.name name of the file used for saving
#' @param baseline.cond.name name of the baseline condition
#' @param condn.name name of the condition which is being compared against baseline
#' @param seq.end end of the sequence or range of the x-axis
#' @param by.step step size of going from 0 to seq.end
#' @param tau a "time-constant" like parameter passed to [find.body.param()]
#' @param dpk threshold for the implicit solution
#' @return NULL
plot.simulated.composites.helper <- function(input.data.identifier="activated.condnvsbaseline", 
					     ...
					     tau = 10, dpk = 1e-5, dataFolder = "../data") {
   print(sprintf("using %s to retrieve files", dataFolder))
   kinit.krel.lists = readRDS(sprintf("%s/kinit.krel.lists.%s.rds", dataFolder, input.data.identifier))
   kinit.vec <- readRDS(sprintf("%s/kinit.vec.%s.rds", dataFolder, input.data.identifier))
   krel.vec <- readRDS(sprintf("%s/krel.vec.%s.rds", dataFolder, input.data.identifier))
   kpre.vec <- readRDS(sprintf("%s/kpre.vec.%s.rds", dataFolder, input.data.identifier))
   kelong.vec <- readRDS(sprintf("%s/kelong.vec.%s.rds", dataFolder, input.data.identifier))
   out.pause <- readRDS(sprintf("%s/out.pause.%s.rds", dataFolder, input.data.identifier))
   out.body <- readRDS(sprintf("%s/out.body.%s.rds", dataFolder, input.data.identifier))
