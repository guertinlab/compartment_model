...
find.pause.regions <- function(bed.input,
    ...
    bed.input[,8] <- bed.input[,7] + 1   # pause.end

                                        #take 1kb window centered around TSS
    tss.df <- bed.input[,c(1,7,8,4,7,6)]  # TSS in tss.df is now pause.start from bed.input
    # thousand base pair window here
...
merge.pbody.deseq2 <- function(df.pause.body, deseq2.df) {
 ...
 merged.df = merge(df.pause.body, deseq2.df, by="row.names", all=T) 
...
#' @return NULL
run.plotting.steps <- function(df.pausebody, cond1.name, bw.cond1.plus, bw.cond1.minus, cond2.name, bw.cond2.plus, bw.cond2.minus, factor.name="efferocytosis", color.names=c(rgb(0,0,1,1/2), rgb(1,0,0,1/2))) {
 ...
 comparison = sprintf("%s.v.%s", trim.cond2.name, trim.cond1.name)
 #df.composite <- readRDS(sprintf("composite.%s.rds", comparison))
 saveRDS(df.composite, file=sprintf("composite.%s.rds", comparison))
 print(sprintf("saved composite.%s.rds", comparison))
 ...
 df.density <- total.region.density(df.pausebody,
                                   ...
                                   bw.cond2.minus,
                                   cond2.name)
 saveRDS(df.density, file=sprintf("density.%s.rds", comparison))
 print(sprintf("saved density.%s.rds", comparison))
